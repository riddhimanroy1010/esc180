...
for index = 1:20
    series_terms(index) = (series_terms(index)) * (1/4)^(index - 1);
end
%summing the series
out = ones(20);
for index = 1:20
...
hold on;
scatter(x, out(:, 1), '*');
legend("Archimedes Series Sums", "Convergence Value")
hold off
...
% legend("Left Handed Sum", "Right Handed Sum", "Exact Solution", "Location", "Southeast");
